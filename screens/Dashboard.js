  ...
  const fetchStockData = async () => {
    try {
      ...
      stockSnapshot.forEach((doc) => {
        ...
        totalPriceStockIn += stock.priceTotalStockIn
          ? stock.priceTotalStockIn
          : 0;
        ...
        totalPriceStockOut += stock.priceTotalStockOut
          ? stock.priceTotalStockOut
          : 0;
      });
    ...
    switch (route.key) {
      case "activity":
        const sortedStocks = stockData
          ...
          .sort(
            (a, b) =>
              b.timeStampUpdatedStock.toDate() -
              a.timeStampUpdatedStock.toDate()
          )
          .slice(0, 4);
        return (
          <View>
                    ...
                    <ListItem.Content>
                      <ListItem.Title>
                        ...
                        <Text style={{ color: "green" }}>
                          {" "}
                          {stock.salesPrice.toLocaleString()}฿
                        </Text>
                      </ListItem.Title>
                      <ListItem.Subtitle style={{ color: "#808080" }}>
                        {stock.timeStampUpdatedStock
                          ? moment(stock.timeStampUpdatedStock.toDate()).format(
                              "llll"
                            )
                          : ""}
                      </ListItem.Subtitle>
      ...
      case "new":
        const sortedNewStocks = stockData
          ...
          .sort(
            (a, b) => b.timeStampProduct.toDate() - a.timeStampProduct.toDate()
          )
          .slice(0, 4);
        return (
          <>
      ...
      case "out":
        const filteredOutStocks = stockData.filter(
          (stock) => stock.quantity < 5
        );
        const sortedOutStocks = filteredOutStocks.sort(
      ...
      >
        ...
        >
          <ColorfulCard
            ...
            footerTitle={
              priceTotalStockIn
                ? `Total ${priceTotalStockIn.toLocaleString()} Bath`
                : 0 + " Bath"
            }
          ...
          <ColorfulCard
            ...
            footerTitle={
              priceTotalStockOut
                ? `Total ${priceTotalStockOut.toLocaleString()} Bath`
                : 0 + " Bath"
            }
            footerValue="for month"
        ...
        >
          <ColorfulCard
            ...
            footerTitle={
              totalPrice ? `Total ${totalPrice.toLocaleString()} Bath` : 0
            }
            footerValue="for all"
